...
class Prometheus:
    ...
    # メイン関数
    def run(self, serial_data: SG.SerialGate, stop_event: threading.Event):
            ...
            while True:
                ...
                if data == PLC_RCV_CMD["CHECK_EXIST"]:
                    ...
                    if flg_judge == JUDGE["OK"]:
                        ...
                        sql_comm.good_vision_update("flawless", timestamp)
                    else:
                        serial_data.send(PLC_SND_CMD["DEFECTIVE"])
                        # 不良品をカウントアップ SQLiteに保存
                        sql_comm.bad_vision_update("defective", timestamp)
                        # 不良品の情報を保存
                        # 未実装 ファイルパスを保存する
                    ##### 判別処理 #####
