...
class SerialConnection:
    ...
    def connect(self):
        try:
            self.serial = serial.Serial(self.port, self.baudrate, bytesize=serial.EIGHTBITS, parity=serial.PARITY_EVEN, stopbits=serial.STOPBITS_ONE, timeout=self.timeout)  # シリアルポートを開く
    ...
    try:        
        ...
        while True:  # メインループ
            if command == "exit":  # コマンドがexitの場合
                ...
                serial_conn1.set_send_word(int(command))  # 最初のシリアルポートにコマンドを設定                
