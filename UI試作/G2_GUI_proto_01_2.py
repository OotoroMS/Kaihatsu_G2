...
class GUI():
    ...
    def __init__(self):
        ...
        self.send_word = None
        #   アフターキャンセル関係
        self.cansel_after_id = None     #   カウント、サイズ、外観検査結果の画面自動更新解除用
        self.rcv_after_id = None        #   繰り返し受信解除用
        self.vitals_after_id = None     #   自動更新のプロセスID(afterの戻り値)
        self.test_after_id = None       #   アフターキャンセル（試験用）
    ...
    def vital_update(self, target_label):
        ...
        if self.vitals_msg == "稼働中":
            ...
            target_label["bg"] = "red"
        self.vitals_after_id = self.rootform.after(1000,self.vital_update,target_label)

    #   稼働状態自動更新を終了
    def vital_update_finish(self):#自動更新(after)が複数同時に立ち上がらないようにするため、画面遷移のたびに古いものを終了させる
        if self.vitals_after_id != None:
            self.rootform.after_cancel(self.vitals_after_id)
            self.vitals_after_id = None

    def vital_update_start(self, target_label):
        self.vital_update_finish()
        self.vital_update(target_label)
    ...
    def vaital_test(self):
        if self.vitals_msg == "稼働中":
            self.vitals_msg = "停止中"
            # self.vitals_msg = Com.change_command(99)#"非常停止中"
        else:
            self.vitals_msg = "稼働中"
        print(self.vitals_msg)
        self.test_after_id = self.rootform.after(5000,self.vaital_test)
    ...
    def recive(self):
        ...
        if data1:
            ...
            self.case_chack()
            self.send()
        self.rcv_after_id = self.rootform.after(1000,self.recive)  # 0.5秒ごとに更新        
    ...
    def admin_mode_frm_create(self):
        if self.admin_mode_frm == None:
            ...
            buttonM1 = tk.Button(self.admin_mode_frm,text="洗浄部",font=("",30))
            buttonM2 = tk.Button(self.admin_mode_frm,text="寸法検査部",font=("",30))
            buttonM3 = tk.Button(self.admin_mode_frm,text="分別部",font=("",30))
            buttonM4 = tk.Button(self.admin_mode_frm,text="蓄積収納部",font=("",30))
            buttonM5 = tk.Button(self.admin_mode_frm,text="外観検査部",font=("",30))
            buttonM6 = tk.Button(self.admin_mode_frm,text="分別部",font=("",30))
            buttonM7 = tk.Button(self.admin_mode_frm,text="蓄積収納部",font=("",30))
    ...
    def frm_destroy(self):
        if self.count_frm != None:          #   カウントログフレームがあれば
            self.count_frm.destroy()        #   フレームを削除
            self.count_frm = None           #   初期化
        elif self.vision_frm != None:       #   会館検査フレームがあれば
            self.vision_frm.destroy()
            self.vision_frm = None
        elif self.size_log_frm != None:     #   サイズログフレームがあれば
            self.size_log_frm.destroy()
            self.size_log_frm = None
        elif self.admin_mode_frm != None:   #   メンテナンスフレームがあれば
            self.admin_mode_frm.destroy()
            self.admin_mode_frm = None
        elif self.work_mode_frm != None:    #   データ一覧フレームがあれば
            self.work_mode_frm.destroy()
            self.work_mode_frm = None
        elif self.main_frm != None:         #   メインフレームがあれば
            self.main_frm.destroy()
            self.main_frm = None
        else:
            print("No frames to destroy.")
    ...
    def vision_up(self):
        ...
        print(self.flag_01)
        if(self.flag_01 == 1):
            self.cansel_after_id = self.rootform.after(1000,self.vision_up)    #1秒ごとに処理
 
    #   グラフを生成
    def graph(self):
        #   データベースからデータを取得
        size_list=[]
        result = self.db.table_data_get('testdb_02',"select * from DB_sizelog order by id desc limit 20")
        for i in result:
    ...
    def show_size_log_graph(self):
        ...
        print(ut)
        if(self.flag_02 == 1):
            self.cansel_after_id = self.rootform.after(5000,self.show_size_log_graph)

    #   当日のカウントログ
    def count_log(self):
        #   自動更新が有効ならば
        if self.cansel_after_id != None:
            self.rootform.after_cancel(self.cansel_after_id)
        ...
        if(self.flag_03 == 1):
            self.cansel_after_id = self.rootform.after(5000,self.count_log)
    ...
    def count_log_7_days(self):
        ...
        #   ﾌﾗｸﾞが立っていたら
        if(self.flag_03 == 1):
            self.cansel_after_id = self.rootform.after(5000,self.count_log_7_days)#後ろで状態監視が動いているのが時々出てくる

    #  終了処理
    def click_close(self):
        if messagebox.askokcancel("確認","終了しますか?"):
            self.vital_update_finish()
            self.rootform.after_cancel(self.rcv_after_id)
            self.rootform.after_cancel(self.test_after_id)
    ...
    def show_main_frm(self):
        ...
        #   稼働状況更新
        self.vital_update_start(self.main_vitals)
    ...
    def show_size_log_frm(self):
        ...
        self.vital_update_start(self.size_vitals)
        self.flag_02 = 1
    ...
    def show_count_frm(self):
        ...
        self.vital_update_start(self.count_vitals)
        self.flag_03 = 1
        self.count_log()
