...
IMAGE_FAIL_PATH = EXPTXT_FILE_PATH + "fail.png"
Y_DISTANCE      = 10
...
class PassPopup(BasePopup):
    ...
    def setting_text(self):
        ...
        for text in self.text_index:
            self.view_texts.append(self.text_font.render(POPUP_TEXTS[text], True, WHITE))

    def setting_images(self):
        # 大きさ
        fail_width  = self.width // 4
        fail_height = self.height // 4
        # 座標
        fail_x = ((self.pos_x  + self.width) // 2) - (fail_width // 2)
        fail_y = self.pos_y + Y_DISTANCE
        self.images = [
            Picture(self.screen, (fail_x, fail_y), (fail_width, fail_height), IMAGE_FAIL_PATH)
        ]
