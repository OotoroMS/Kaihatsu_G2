...
from DATABASE.SQLCommunication import SQLCommunication as DC
...
SQL = {
    ...
    "error" : "select id,datetime from db_timelog order by id DESC limit 50"
}
TODAY = {
    "px" : 420,#配置
    "py" : 200,
...
def _get_data(target):
    data = list()
    if target in SQL:
        result = DB.table_data_get(DATABESE, SQL[target])
        if target != "error":
            for i in result:
