...
pygame.display.set_caption("Slide Transition with Easing")
...
# スライドアニメーション用の変数
slide_duration = 25  # スライドのフレーム数（アニメーションの長さ）
...
def draw_main_menu(offset_x=0):
    screen.blit(background_image_main, (offset_x, 0))
    title_text = font.render("Main Menu", True, BLUE)
    screen.blit(title_text, (screen_width//2 - title_text.get_width()//2 + offset_x, screen_height//3 - title_text.get_height()//2))
    draw_button("Start Game", screen_width//2 - 100 + offset_x, screen_height//2, 200, 60)

# ゲーム画面の描画
def draw_game_screen(offset_x=0):
    screen.blit(background_image_game, (offset_x, 0))
    game_text = font.render("Game Screen", True, WHITE)
    screen.blit(game_text, (screen_width//2 - game_text.get_width()//2 + offset_x, screen_height//3 - game_text.get_height()//2))
    draw_button("Back to Menu", screen_width//2 - 150 + offset_x, screen_height//2, 300, 60)
...
def slide_transition(current, next):
    ...
    for frame in range(slide_duration + 1):
        ...
        # イージングを使ってスライド量を計算
        slide_offset = ease_in_out_quad(frame, 0, screen_width, slide_duration)
