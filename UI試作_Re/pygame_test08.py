...
pygame.display.set_caption("Smooth Darken Transition Example")
...
def transition_to_next_screen():
    global is_transitioning, current_screen, next_screen
    ...
    for frame in range(DARKEN_DURATION):
        ...
        screen.blit(background_image_main, (0, 0))  # 背景を描画
    ...
    # 暗いオーバーレイを消す
    overlay.set_alpha(255)  # オーバーレイを完全に暗く
    screen.blit(overlay, (0, 0))
    pygame.display.flip()
    pygame.time.delay(100)  # 短い待機時間を追加

    # オーバーレイを明るくする処理（完全に消す）
    overlay.set_alpha(0)
    for frame in range(DARKEN_DURATION):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
